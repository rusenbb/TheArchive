...
# ----------- NEW IMPORTS FOR SETTING ENTITIES IN GAZEBO -----------
...
class RobotController(Node):
    def __init__(self):
        ...
        # - "MOVING_FORWARD": moving 0.6 m forward
        # - "TURNING_CW": turning 90 deg right
        # - "WAITING_NO_PERSON": wait for 2 consecutive frames with no "person-box" > 0.5
        # - "STOPPING_FOR_CROSSWALK": if offset not in [-300, 300], area>100000, wait for either
        #   2 consecutive frames no person or 15 seconds, or area < 100000
        #
        ...
        # For counting consecutive frames with no detection
        self.consecutive_no_person_frames = 0
        
        # For detecting "person-box" triggers
        self.person_box_detected_this_frame = False
        
        # For checking if we need to do the 90° CCW sequence (the "area gets bigger in 2 consecutive frames" logic)
        self.last_in_range_area = 0.0
        self.consecutive_area_increases = 0
        
        # Crosswalk stopping logic
        self.crosswalk_stop_start_time = None  # Track time when we enter STOPPING_FOR_CROSSWALK
        self.crosswalk_time_limit = 15.0       # 15 seconds limit for crosswalk wait
        
        # Store the largest person area (for the current frame) to help with resume conditions
    ...
    def control_loop(self):
        """Main control loop for robot movement including special 'person-box' behaviors."""
        ...
        if self.action_state == "PATROLLING":
            # If we've just detected that we should stop for crosswalk
            if self.crosswalk_stop_start_time is not None:
                self.get_logger().info("Stopping for crosswalk detected. Switching to STOPPING_FOR_CROSSWALK.")
                self.action_state = "STOPPING_FOR_CROSSWALK"
            # If we've just detected that we should do the 90° CCW turn sequence
            elif self.person_box_detected_this_frame:
                self.get_logger().info("Detected 'person-box' in-range & area grown for 2 consecutive frames; switching to TURNING_CCW.")
                ...
                # Reset no-person count
                self.consecutive_no_person_frames = 0
            else:
                # Continue normal patrol
                self.do_patrol_movement(cmd_vel)
        
        elif self.action_state == "STOPPING_FOR_CROSSWALK":
            # Robot stops
            cmd_vel.linear.x = 0.0
            cmd_vel.angular.z = 0.0

            # Check if we are done waiting:
            # Condition 1: area < 100000 => resume
            # Condition 2: 2 consecutive frames no person => resume
            # Condition 3: Time limit 15s => resume
            if self.largest_person_area < 100000:
                self.get_logger().info("Person area < 100000, resuming patrol.")
                self.action_state = "PATROLLING"
                self.crosswalk_stop_start_time = None
            elif self.consecutive_no_person_frames >= 1:
                self.get_logger().info("No person-box for 2 consecutive frames. Resuming patrol.")
                self.action_state = "PATROLLING"
                self.crosswalk_stop_start_time = None
            else:
                now_sec = self.get_clock().now().nanoseconds / 1e9
                if (now_sec - self.crosswalk_stop_start_time) > self.crosswalk_time_limit:
                    self.get_logger().info("Crosswalk wait exceeded 15s. Resuming patrol.")
                    self.action_state = "PATROLLING"
                    self.crosswalk_stop_start_time = None
            ...
            if abs(angle_diff) > self.angular_tolerance:
                cmd_vel.angular.z = 1.5  # turning left
                cmd_vel.linear.x = 0.0
            else:
                # Done turning
                self.get_logger().info("Finished 90° CCW turn, now moving forward 0.6m.")
                self.action_state = "MOVING_FORWARD"
                # Record the start position so we can measure 0.6m
            ...
            if dist_moved < 0.8: 
                ...
                cmd_vel.linear.x = 0.3  # You can adjust this speed as necessary
                cmd_vel.angular.z = 0.0
            else:
                # Done moving 0.6m
            ...
            if abs(angle_diff) > self.angular_tolerance:
                ...
                self.get_logger().info("Finished 90° CW turn, waiting for no person-box in 2 consecutive frames.")
                self.action_state = "WAITING_NO_PERSON"
                self.consecutive_no_person_frames = 0
        
        elif self.action_state == "WAITING_NO_PERSON":
            # Robot stops and waits until no person-box is detected for 2 consecutive predictions
            # OR if the person's area has dropped below 100,000
            cmd_vel.linear.x = 0.0
            cmd_vel.angular.z = 0.0
            ...
            if self.largest_person_area < 100000:
                ...
                self.get_logger().info("No 'person-box' > 0.5 for 2 consecutive frames. Resuming patrol.")
                self.action_state = "PATROLLING"
        ...
        # Reset the detection flags (for area growth) each cycle
        self.person_box_detected_this_frame = False
    ...
    def box_detected_callback(self, msg):
        """
        Receives YOLO predictions as a JSON string and applies the new logic:
        
        1) Check for a "person-box" with conf>0.5 and area>100000.
           - If offset.x in [-300,300], track if area is increasing in consecutive frames.
             If we've had 2 consecutive increases, set a flag to do the 90° CCW behavior
             (only if in PATROLLING).
           - If offset.x is not in [-300,300], trigger crosswalk stop (only if in PATROLLING).
        
        2) Count how many consecutive frames have NO person-box>0.5 (including no predictions).
           This is used for both WAITING_NO_PERSON (after turn) and STOPPING_FOR_CROSSWALK.
        
        3) We also record the largest person area (self.largest_person_area) to allow
           resuming normal patrol if it falls below 100,000.
        """
        try:
        ...
        # By default, assume we detect no persons in this frame
        found_person_box_this_frame = False
        
        # We'll track the largest in-range area for "person-box" in offset [-300,300]
        largest_in_range_area = 0.0
        
        # We'll track if there's a crosswalk scenario
        ...
        for box in data:
            if 'object_name' in box and 'confidence' in box:
                if box['object_name'] == 'person-box' and box['confidence'] > 0.5:
                    ...
                    # (offset outside [-300, 300] and area>100000)
                    if abs(offset_x) > 300 and area > 100000:
                        crosswalk_trigger = True
        
        # Update consecutive_no_person_frames
        if found_person_box_this_frame:
            # Reset the no-person counter
            self.consecutive_no_person_frames = 0
        else:
            # Increase the no-person counter
            self.consecutive_no_person_frames += 1
        
        # Handle crosswalk scenario:
        # If area>100000, offset NOT in [-300,300], and we are in PATROLLING => trigger crosswalk stop
        if crosswalk_trigger and self.action_state == "PATROLLING":
            # Mark the time if not already stopping
            if self.crosswalk_stop_start_time is None:
                self.crosswalk_stop_start_time = self.get_clock().now().nanoseconds / 1e9

        # Handle in-range "person-box" with area>100000 to check for area growth
        if self.action_state == "PATROLLING" and largest_in_range_area > 0:
            # Compare with last_in_range_area
            ...
            # If area has grown in 2 consecutive frames, set a flag
            if self.consecutive_area_increases >= 2:
                self.person_box_detected_this_frame = True
                # Once triggered, you might want to reset the counters 
                self.consecutive_area_increases = 0
        else:
            # If no in-range detection, reset area counters
            self.consecutive_area_increases = 0
            self.last_in_range_area = 0.0


# ---------------
# FUNCTION TO SET ENTITIES IN GAZEBO BEFORE ANYTHING ELSE
# ---------------
def set_entities_positions(node, positions_dict):
    ...
    for name, coords in positions_dict.items():
        ...
        # If your world is flat, you might set z = 0.0 explicitly.
        entity_state.pose.position.z = 0.0
        # Orientation defaults to (0,0,0,1).
