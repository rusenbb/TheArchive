...
# Prepared for ITU BBF YZV Robotics course, 24-25 Fall Semester, Term Project
...
class YoloInferenceNode(Node):
    def __init__(self, frame_skip=5):
        ...
        self.frame_save_dir = f'/root/frames/raw/'
        self.yolo_frame_save_dir = f'/root/frames/yolo/'
        os.makedirs(self.frame_save_dir, exist_ok=True)
        os.makedirs(self.yolo_frame_save_dir, exist_ok=True)

        self.get_logger().info(f'Frames will be saved to {self.frame_save_dir}')
        self.get_logger().info(f'YOLO frames will be saved to {self.yolo_frame_save_dir}')
    ...
    def image_callback(self, msg: Image):
        ...
        try:
            # Convert ROS Image message to OpenCV image (raw frame)
            cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
            # Always save the raw image
            cv2.imwrite(f'{self.frame_save_dir}frame_{self.frame_counter}.jpg', cv_image)
        except CvBridgeError as e:
            self.get_logger().error(f'CvBridge while saving Error: {e}')
            return

        # Skip frames if not at the correct count
        ...
        try:
            # Convert ROS Image message again to OpenCV (for inference)
            cv_image = self.bridge.imgmsg_to_cv2(msg, desired_encoding='bgr8')
        except CvBridgeError as e:
        ...
        if results:
            # results[0].plot() returns an annotated image (OpenCV format)
            annotated_image = results[0].plot()
            cv2.imwrite(f'{self.yolo_frame_save_dir}yolo_frame_{self.frame_counter}.jpg', annotated_image)

        # Publish predictions if we have any
        if predictions_list:
...
class RobotController(Node):
    ...
    def control_loop(self):
        ...
        # 1) Check and handle human avoidance states first.
        #    If avoidance is active, skip the rest of the logic this cycle.
        if self.human_avoidance(cmd_vel):
            self.last_cmd_vel = cmd_vel
            self.vel_publisher.publish(cmd_vel)
            return

        # Handle normal states if no human avoidance is active
        # ----------------------------------------------------------------
        if self.action_state == "PATROLLING":
            ...
            if self.blue_box_target is None:
                # If the target box is no longer valid, resume patrol
                self.action_state = "PATROLLING"
            else:
            ...
            # Navigate to the delivery point or door
            if not self.reached_delivery_patrol_point:
                # Follow patrol route until the delivery waypoint is reached
                self.do_patrol_movement(cmd_vel)
                ...
                if distance < 1.0:
                    ...
                    # Resume normal patrol from the delivery point
                    self.action_state = "PATROLLING"
    ...
    def handle_avoidance_states(self, cmd_vel):
        """
        handle the active human-avoidance state and set cmd_vel accordingly.
        ...
        if self.action_state == "STOPPING_FOR_CROSSWALK":
            ...
            # 2 consecutive frames no person => done
...
def main(args=None):
    ...
    yolo_node = YoloInferenceNode(frame_skip=5)  
